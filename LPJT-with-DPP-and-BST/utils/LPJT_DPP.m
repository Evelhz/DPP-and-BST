function [accVec] = LPJT_DPP(DataArr,options)
Xs = DataArr.Xs;
Xt = DataArr.Xt;
Ys = DataArr.Ys;
Yt = DataArr.Yt;
...
for i = 1:T
    ...
    Smin = [Mss+gamma*Sws+lambda*eye(m)+eta*L,Mst-lambda*eye(m);Mts-lambda*eye(m),Mtt+gamma*Swt+(lambda+mu)*eye(m)];
    ...
    accVec(i) = sum(Yt0==Yt)/numel(Yt)*100;
